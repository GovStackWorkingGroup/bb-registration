...
Feature: API endpoint that allows to upload a required document
  ...
  Scenario: Successfully uploaded a document smoke type test
    ...
    When POST request to upload a document with given "V2UgbG92ZSBqc29uIQ==" as an item of the file array is sent
  ...
  Scenario Outline: Successfully uploaded documets
    ...
    Given Wants to upload a document
    When POST request to upload a document with given "<document1>" and "<document2>" as items of the file array is sent
    And The request contains "<metadata>" as additionalMetadata
    Then The response from the /documents endpoint is received
    And The /documents response should be returned in a timely manner 15000ms
    And The /documents response should have status 200
    And The /documents response should match json schema
    ...
    Examples:
    | document1                                     | document2                                     | metadata      | 
    | VXBsb2FkZWQgZG9jdW1lbnQgYmFzZWQ2NA==          | YW5vdGhlciBkb2N1bWVudCB1c2VkIGluIHRlc3RzIA==  | additional    | 
    | V2UgbG92ZSBqc29uIQ==                          | ZG9jdW1lbnQgb3IgYXR0YWNobWVudA==              | metadata      |
